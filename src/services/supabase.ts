...
export async function getLMSPosts(classSection: string, subject?: string) {
  let query = supabase.from('lms_posts').select('*').eq('class_section', classSection);
  if (subject) query = query.eq('subject', subject);
  const { data, error } = await query.order('created_at', { ascending: false });
  if (error) { console.error('Error fetching LMS posts:', error); return []; }
  return data as LMPost[];
}

export async function createLMSPost(post: Omit<LMPost, 'id' | 'created_at'>) {
  const { data, error } = await supabase.from('lms_posts').insert([post]).select().single();
  if (error) { console.error('Error creating LMS post:', error); throw error; }
  return data as LMPost;
}

export async function getLMMeetings(classSection: string, teacherId?: number) {
  let query = supabase.from('lms_meetings').select('*').eq('class_section', classSection);
  if (teacherId) query = query.eq('teacher_id', teacherId);
  const { data, error } = await query.order('start_time', { ascending: true });
  if (error) { console.error('Error fetching meetings:', error); return []; }
  return data as LMMeeting[];
}

export async function scheduleMeeting(meeting: Omit<LMMeeting, 'id' | 'created_at'>) {
  const { data, error } = await supabase.from('lms_meetings').insert([meeting]).select().single();
  if (error) { console.error('Error scheduling meeting:', error); throw error; }
  return data as LMMeeting;
}

export async function getLMAssignments(classSection: string, subject?: string) {
  let query = supabase.from('lms_assignments').select('*').eq('class_section', classSection);
  if (subject) query = query.eq('subject', subject);
  const { data, error } = await query.order('created_at', { ascending: false });
  if (error) { console.error('Error fetching assignments:', error); return []; }
  return data as LMAssignment[];
}

export async function submitAssignment(submission: Omit<LMSubmission, 'id' | 'submitted_at'>) {
  const { data, error } = await supabase.from('lms_submissions').insert([submission]).select().single();
  if (error) { console.error('Error submitting assignment:', error); throw error; }
  return data as LMSubmission;
}

export async function getSubmissions(assignmentId: string) {
  const { data, error } = await supabase.from('lms_submissions').select('*').eq('assignment_id', assignmentId);
